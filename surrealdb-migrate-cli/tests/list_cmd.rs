...
#[tokio::test]
async fn list_migrations_none_applied() {
    ...
    cmd.assert()
        .code(0)
        .stdout_eq(file!("list_cmd/forward_migrations_none_applied.stdout"))
        .stderr_eq("");
}

#[tokio::test]
async fn list_forward_migrations_none_applied() {
    let db_server = start_surrealdb_testcontainer().await;
    let db_config = prepare_test_database(&db_server).await;
...
#[tokio::test]
async fn list_backward_migrations_none_applied() {
    let db_server = start_surrealdb_testcontainer().await;
    let db_config = prepare_test_database(&db_server).await;
    ...
    cmd.assert()
        .code(0)
        .stdout_eq(file!("list_cmd/backward_migrations_none_applied.stdout"))
        .stderr_eq("");
}
...
#[tokio::test]
async fn list_forward_and_backward_migrations_none_applied() {
    let db_server = start_surrealdb_testcontainer().await;
    let db_config = prepare_test_database(&db_server).await;
    ...
    cmd.assert()
        .code(0)
        .stdout_eq(file!(
            "list_cmd/forward_and_backward_migrations_none_applied.stdout"
        ))
        .stderr_eq("");
}

#[tokio::test]
async fn list_forward_and_backward_migrations_one_applied() {
    let db_server = start_surrealdb_testcontainer().await;
    let db_config = prepare_test_database(&db_server).await;
    let db = connect_to_test_database_as_database_user(&db_config).await;
    ...
    cmd.assert()
        .code(0)
        .stdout_eq(file!(
            "list_cmd/forward_and_backward_migrations_one_applied.stdout"
        ))
        .stderr_eq("");
}

#[tokio::test]
async fn list_open_forward_and_backward_migrations_one_applied() {
    let db_server = start_surrealdb_testcontainer().await;
    let db_config = prepare_test_database(&db_server).await;
    let db = connect_to_test_database_as_database_user(&db_config).await;
    ...
    cmd.assert()
        .code(0)
        .stdout_eq(file!(
            "list_cmd/open_forward_and_backward_migrations_one_applied.stdout"
        ))
        .stderr_eq("");
}

#[tokio::test]
async fn list_applied_forward_and_backward_migrations_one_applied() {
    let db_server = start_surrealdb_testcontainer().await;
    let db_config = prepare_test_database(&db_server).await;
    let db = connect_to_test_database_as_database_user(&db_config).await;
    ...
    cmd.assert()
        .code(0)
        .stdout_eq(file!(
            "list_cmd/applied_forward_and_backward_migrations_one_applied.stdout"
        ))
        .stderr_eq("");
}
